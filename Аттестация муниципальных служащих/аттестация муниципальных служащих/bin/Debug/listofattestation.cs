    ...
    public partial class listofattestation : Form
    {
        ...
        private async void RefreshGrid()
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    // Привязка данных (из Page 2 вашего PDF)
        ...
        public void SetData(string num, string fio, string stat, string otdel, string ph, string sAtt, string t)
        {
            try
            {
                ...
                {
                    conn.Open();
                    // ИСПРАВЛЕНО: Убрали NumOfAtt из INSERT, так как это IDENTITY поле (Page 4 PDF)
        ...
        // ВОССТАНОВЛЕН ПОИСК (Page 7-8 PDF)
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dt != null)
            {
                ...
                dt.DefaultView.RowFilter = string.Format(
                    ...
                    "FIO LIKE '%{0}%' OR Status LIKE '%{0}%' OR Otdel LIKE '%{0}%' OR " +
                    "Phone LIKE '%{0}%' OR StatOfAtt LIKE '%{0}%' OR Time LIKE '%{0}%'",
                    search);
            }
        }

        // ВОССТАНОВЛЕНА ОЧИСТКА (Page 8 PDF)
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = "DELETE FROM [Table]"; // Безопаснее чем TRUNCATE
                    SqlCommand cmd = new SqlCommand(query, conn);
                    cmd.ExecuteNonQuery();
                }
                RefreshGrid();
        ...
        // ВОССТАНОВЛЕНЫ ПУНКТЫ МЕНЮ (Page 9 PDF)
        private void добавитьПротоколАттестацииToolStripMenuItem_Click(object sender, EventArgs e)
        {
